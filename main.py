    ...
    if not config['quiet']:
        if col:
            if config['colors']:
                ...
                sys.stderr.write(col + x + '\n')
        else:
            if config['colors']:
                sys.stderr.write(x + bcolors.ENDC + '\n')
            else:
                sys.stderr.write(x + '\n')
...
class ScriptObfuscator:
    ...
    def __init__(self, normalize_only=False, reserved_words=[], garbage_perc=12.0, min_var_length=5):
        ...
        self.bitShuffleObfuscator = BitShuffleStringObfuscator(ScriptObfuscator.obfuscateChar,
                                                               ScriptObfuscator.obfuscateNumber)
    ...
    def detectFunctionBoundaries(self):
        ...
        for m in re.finditer(ScriptObfuscator.FUNCTION_REGEX, self.output, flags=re.I | re.M):
            ...
            if counter > 0:
                ...
                info("Function boundaries: (%s, from: %d, to: %d)" % (
                prev['funcName'], prev['funcStart'], prev['funcStop']))
    ...
    def mergeAndConcatLongLines(self, txt):
        ...
        while True:
            ...
            varName, pos, lineStart, lineStop, origLine, longLine))
            newLine = ''
        ...
        for (orig, new, longLine) in replaces:
            if DEBUG:
                ...
                info("Merging long string line (var: %s, len: %d): '%s...%s'" % (
                varName, len(longLine), longLine[:40], longLine[-40:]))
    ...
    def randomizeVariablesAndFunctions(self):
        ...
        def replaceVar(name, m):
            ...
            if m and len(m.groups()) > 0:
                ...
                info(name + " name obfuscated (context: \"%s\"): '%s' => '%s'" % (
                m.group(0).strip(), varToReplace, varName))
        ...
        for m in re.finditer(ScriptObfuscator.FUNCTION_REGEX, self.output, flags=re.I | re.M):
            ...
            info("Function name obfuscated (context: \"%s\"): '%s' => '%s'" % (
            m.group(0).strip(), varToReplace, varName))
        ...
        for (varToReplace, varName) in replacedAlready.items():
            ...
            self.output = re.sub(r"(?<!\.)(?:\b" + varToReplace + r"\b)|(?:\b" + varToReplace + r"\s*=\s*)", varName,
                                 self.output, flags=re.I | re.M)
    ...
    def obfuscateStrings(self, useBitShuffler=True):
        ...
        for m in re.finditer(ScriptObfuscator.STRINGS_REGEX, self.output, flags=re.I | re.M):
            ...
            if BitShuffleStringObfuscator.STRING_PADDING_CHAR in string:
                info(
                    "\tPadding character: (%s) has been detected in input string. Have to avoid Bit Shuffle string encoder." % BitShuffleStringObfuscator.STRING_PADDING_CHAR)
        ...
        for m in re.finditer(r"\bArray\s*\(([^\)]+?)\)", self.output, flags=re.I | re.M):
            ...
            if all(map(lambda x: x in for_nums_allowed, array)):
                ...
                try:
                    ...
                    info("\tObfuscated array: Array(%s, ..., %s)" % (
                    ','.join(new_array)[:40], ','.join(new_array)[-40:]))
...
''')
    ...
    group2.add_argument("-N", "--normalize", dest="normalize_only",
                        help="Don't perform obfuscation, do only code normalization (like long strings transformation).",
                        action='store_true')
    group2.add_argument("-g", "--garbage", help="Percent of garbage to append to the obfuscated code. Default: 12%%.",
                        default=config['garbage_perc'], type=float)
    ...
    parser.add_argument("-m", "--min-var-len", dest='min_var_len',
                        help="Minimum length of variable to include in name obfuscation. Too short value may break the original script. Default: 5.",
                        default=config['min_var_length'], type=int)
    parser.add_argument("-r", "--reserved", action='append',
                        help='Reserved word/name that should not be obfuscated (in case some name has to be in original script cause it may break it otherwise). Repeat the option for more words.')
